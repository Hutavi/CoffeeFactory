...
function ForgetPassword(props) {
  ...
  const handleEmailConfirm = (event) => {
    ...
    axios.post('http://192.168.1.55:3000/tcf/v1/users/forgotPassword', object).then((res) => {
      handleSetStep(2)
  ...
  const handlePinConfirm = (event) => {
    ...
    axios.post('http://192.168.1.55:3000/tcf/v1/users/validateCode', object).then((res) => {
      handleSetStep(3)
      setToken(res.data.token)
    }).catch(error => {
      setPinWrong(true)
  ...
  const handlePasswordConfirm = (event) => {
    ...
    axios.patch('http://192.168.1.55:3000/tcf/v1/users/resetPassword', object).then((res) => {
      setStep(4)
    }).catch(error => {
      let a = error.response.data.message.split('. ')
    ...
    <div className='ForgetPassword'>
      {step === 1 && <form onSubmit={handleEmailConfirm} className='step1'>
        ...
        <div className='optionsAccount' style={{ marginTop: '-25px' }}>
          <p style={{ color: 'red', fontWeight: '700', cursor: 'pointer' }} onClick={() => setClick(2)}>Đăng ký</p>
          <p style={{ color: 'red', fontWeight: '700', cursor: 'pointer' }} onClick={() => setClick(1)}>Đăng nhập</p>
        </div>
        <div className='backIcon' onClick={() => setClick(1)}>
